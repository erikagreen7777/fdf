...
static void	first_maps(t_data *raw, t_coord **data)
{
	...
	{
		data[x][y].z = ft_atoi(raw->str[y]);
		data[x][y].x = (((y * cos(ZANG) - x * sin(ZANG)) * 20)) + ((600) - (raw->width * 10));
...
static void	map_picker(t_data *raw, t_coord **data)
{
	if (ft_strstr(raw->name, "maps/42.fdf") || ft_strstr(raw->name, "maps/10-2.fdf") || ft_strstr(raw->name, "maps/basictest.fdf") ||
		...
		hundred(raw, data);
	else if (ft_strstr(raw->name, "maps/20-60.fdf")  || ft_strstr(raw->name, "maps/10-70.fdf"))
		twenty(raw, data);
	else if (ft_strstr(raw->name, "maps/50-4.fdf") || ft_strstr(raw->name, "maps/pylone.fdf"))
		fifty(raw, data);
	else if (ft_strstr(raw->name, "maps/mars.fdf"))
		mars(raw, data);
	else if (ft_strstr(raw->name, "maps/pentenegpos.fdf") || ft_strstr(raw->name, "maps/plat.fdf") || ft_strstr(raw->name, "maps/pnp_flat.fdf"))
		pees(raw, data);
